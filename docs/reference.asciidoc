...
The structural under operator takes two functions: When called as `A⍢B x`, the function `B` is first called on `x`, and the return value is passed to `A`.
The effect of `B` is then reversed.
