...
class LoadTest : APLTest() {
    ...
    fun ensureLoadPreservesOldNamespace() {
        val engine = Engine()
        val res0 = engine.parseAndEval(StringSourceLocation("namespace(\"a\") x←1 ◊ io:load \"test-data/test-source.kap\""))
        assertSimpleNumber(10, res0)
        val res1 = engine.parseAndEval(StringSourceLocation("foo:bar 1"))
        assertSimpleNumber(101, res1)
    ...
    fun ensureLoadPreservesOldNamespaceOnError() {
        val engine = Engine()
        try {
            engine.parseAndEval(StringSourceLocation("namespace(\"a\") x←1 ◊ io:load \"test-data/parse-error.kap\""))
