...
class InverseFnTest : APLTest() {
    ...
    @Test
    fun subInverse0() {
        assertSimpleNumber(1, parseAPLExpression("7 -˝ 6"))
    }

    @Test
    fun subInverse1() {
        assertSimpleNumber(15, parseAPLExpression("8 -⍨˝ 7"))
    }
    ...
    fun reciprocalInverse() {
        assertRational(Rational.make(1, 8), parseAPLExpression("÷˝ 8"))
    }

    @Test
    fun divInverse0() {
        assertSimpleNumber(32, parseAPLExpression("128 ÷˝ 4"))
    }

    @Test
    fun divInverse1() {
        assertSimpleNumber(8192, parseAPLExpression("4 ÷⍨˝ 2048"))
    }

    @Test
    fun mulInverse0() {
        assertSimpleNumber(5, parseAPLExpression("2 ×˝ 10"))
    }

    @Test
    fun mulInverse1() {
        assertRational(Rational.make(1, 5), parseAPLExpression("10 ×⍨˝ 2"))
    }
    ...
    @Test
    fun addInverse1Arg1() {
        assertSimpleComplex(Complex(4.0, -9.0), parseAPLExpression("+˝ 4J9"))
    }

    @Test
    fun addInverseWithSwap() {
        assertSimpleNumber(7, parseAPLExpression("1 +⍨˝ 8"))
    }

    @Test
    fun logInverse1ArgWithArray() {
        parseAPLExpression("⍟˝ 2 3 ⍴ 2+⍳6").let { result ->
            assertDimension(dimensionsOfSize(2, 3), result)
            assertArrayContent(
                arrayOf(
    ...
    fun addInverse2ArgWithArray() {
        parseAPLExpression("2 +˝ 2 3 ⍴ 100+⍳6").let { result ->
            assertDimension(dimensionsOfSize(2, 3), result)
    ...
    fun leftAssignedInverseTwoArgFails() {
        assertFailsWith<LeftAssigned2ArgException> {
            parseAPLExpression("7 ((1+)˝) 8")
        }
    }

    @Test
    fun inverseWithMonadicChain0() {
        assertSimpleNumber(50, parseAPLExpression("(2+200÷)˝ 6"))
    }
    ...
    fun inverseWithMonadicChain2() {
        assertRational(Rational.make(1, 8), parseAPLExpression("((2+)÷)˝10"))
    }

    @Test
    fun inverseWithDyadicChain0() {
        assertSimpleDouble(25.0, parseAPLExpression("15 (÷+)˝0.025"))
    }

    @Test
    fun inverseExponential0() {
        assertAPLValue(NearDouble(9.0, 4), parseAPLExpression("2 *˝512"))
    }

    @Test
    fun inverseExponential1() {
        assertAPLValue(NearDouble(1.609437912, 8), parseAPLExpression("*˝5"))
    }
    ...
    @Test
    fun inverseLog1() {
        assertRational(Rational.make(1, 8), parseAPLExpression("2 ⍟˝¯3"))
    }
