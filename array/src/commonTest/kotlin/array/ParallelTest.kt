...
class ParallelTest : APLTest() {
    ...
    fun singletonForeach() {
        parseAPLExpression("{1+⍵}¨∥ 10", numTasks = 10).let { result ->
    ...
    fun singletonForeachIsCollapsed() {
        val (result, out) = parseAPLExpressionWithOutput("{io:print ⍵ ⋄ 1+⍵}¨∥ 10", numTasks = 1)
        assertSimpleNumber(11, result)
        assertEquals("10", out)
    }
    ...
    fun singletonForeachWithEnclosedIsCollapsed() {
        val (result, out) = parseAPLExpressionWithOutput("{10+⍵}¨∥ ⊂{io:print ⍵ ⋄ 1+⍵}¨10 20 30", numTasks = 10)
    ...
    fun arraySmallerThanTasks() {
        parseAPLExpression("{1+⍵}¨∥ 10 11 12 13 14 15", numTasks = 10).let { result ->
    ...
    @Test
    fun largeArray() {
        parseAPLExpression("{1+⍵}¨∥ ⍳10000", numTasks = 10).let { result ->
    ...
    fun exceptionInParallelTask() {
        assertFailsWith<APLEvalException> {
            parseAPLExpression("{if(⍵=30) { io:print (1 2)[5] } ⋄ 1+⍵}¨∥ ⍳100", withStandardLib = true, numTasks = 10)
        }
    }
    ...
    fun exceptionInSingletonForeach() {
        assertFailsWith<APLEvalException> {
            parseAPLExpression("{io:print (1 2)[5] ⋄ 1+⍵}¨∥ 10", withStandardLib = true, numTasks = 10)
        }
    }

    @Test
    fun singletonForeachTwoArg() {
        parseAPLExpression("11 {⍺+⍵}¨∥ 10", numTasks = 10).let { result ->
    ...
    fun singletonForeachTwoArgIsCollapsed() {
        val (result, out) = parseAPLExpressionWithOutput("11 {io:print ⍺ ⋄ io:print ⍵ ⋄ ⍺+⍵}¨∥ 10", numTasks = 1)
        assertSimpleNumber(21, result)
    ...
    fun singletonForeachWithEnclosedTwoArgIsCollapsed() {
        val (result, out) = parseAPLExpressionWithOutput("100 {⍺+⍵}¨∥ ⊂{io:print ⍵ ⋄ 1+⍵}¨10 20 30", numTasks = 10)
        assertTrue(result.isScalar())
        val inner = result.valueAt(0)
        assertDimension(dimensionsOfSize(3), inner)
    ...
    fun arraySmallerThanTasksTwoArg() {
        parseAPLExpression("100 200 300 400 500 600 {⍺+⍵}¨∥ 10 20 30 40 50 60", numTasks = 10).let { result ->
            assertDimension(dimensionsOfSize(6), result)
    ...
    fun largeArrayTwoArg() {
        parseAPLExpression("(1+⍳10000) {⍺+⍵}¨∥ ⍳10000", numTasks = 10).let { result ->
            assertDimension(dimensionsOfSize(10000), result)
            repeat(10000) { i ->
