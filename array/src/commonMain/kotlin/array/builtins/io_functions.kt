...
class HttpRequestFunction : APLFunctionDescriptor {
    class HttpRequestFunctionImpl(pos: FunctionInstantiation) : NoAxisAPLFunction(pos) {
        override fun eval1Arg(context: RuntimeContext, a: APLValue): APLValue {
            ...
            val headers = when (args.listSize()) {
                1 -> emptyMap()
                2 -> ensureHeaderArray(args.listElement(1), pos)
                else -> throwAPLException(
                    APLIllegalArgumentException(
                        "Function requires 1-2 arguments, ${args.listSize()} arguments were passed.",
                        pos))
            }
            val result = httpRequest(url, headers)
