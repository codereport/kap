...
If a function isn't scalar, then it's structural which means that the behaviour of the function does not follow the simple rules outlined above.
...
The `when` statement can be used instead of a series of `if` and `else` statements.
